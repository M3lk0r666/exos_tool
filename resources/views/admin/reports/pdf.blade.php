...
<head>
    ...
    <style>
        ...
        * { font-family: DejaVu Sans, sans-serif; }
        ...
        h2 { font-size: 15px; color: #1d4ed8; border-bottom: 2px solid #1d4ed8; padding-bottom: 4px; margin: 22px 0 10px; }
        ...
        .cover img { max-height: 70px; margin: 0 25px; }
        .cover .subtitle { font-size: 14px; color: #6b7280; margin-bottom: 30px; }
        .cover table { width: 70%; margin: 30px auto 0; }
        .page-break { page-break-after: always; }
        .badge { display: inline-block; padding: 2px 8px; border-radius: 8px; color: #fff; font-size: 9px; font-weight: bold; }
        .muted { color: #6b7280; }
        .evidence { background: #f3f4f6; border: 1px solid #d1d5db; border-radius: 4px; padding: 6px; font-family: DejaVu Sans Mono, monospace; font-size: 8.5px; white-space: pre-wrap; word-wrap: break-word; }
        .bar-track { background: #e5e7eb; width: 100%; height: 12px; border-radius: 3px; }
        .bar { height: 12px; border-radius: 3px; }
        .finding { margin-bottom: 14px; page-break-inside: avoid; border: 1px solid #e5e7eb; border-radius: 6px; padding: 10px; }
        .finding h3 { margin: 0 0 6px; }
        .area-cell { font-weight: bold; color: #fff; text-align: center; border-radius: 4px; }
        .foot-note { font-size: 9px; color: #6b7280; }
        .attachment-img { max-width: 300px; max-height: 200px; margin: 4px 8px 4px 0; border: 1px solid #d1d5db; }
...
@php
    ...
    $logo = function (?string $path) {
        if (! $path) return null;
        $full = str_starts_with($path, 'client-logos') || str_starts_with($path, 'branding')
...
<div class="cover">
    <div class="logos">
        @if ($logo($client?->logo_path))<img src="{{ $logo($client->logo_path) }}" alt="cliente">@endif
    </div>
    ...
    <table>
        <tr><th style="width:35%">Cliente</th><td>{{ $client?->name ?? '—' }}</td></tr>
        <tr><th>Equipo</th><td>{{ $device?->displayName() ?? '—' }} ({{ $summary['system_type'] ?? '' }})</td></tr>
        <tr><th>Fecha de la captura</th><td>{{ $capture->captured_at?->format('d/m/Y H:i') ?? '—' }}</td></tr>
        <tr><th>Versión del reporte</th><td>v{{ $report->version }} — {{ $report->status->label() }}</td></tr>
        <tr><th>Emitido por</th><td>{{ $report->issuer?->name ?? 'Borrador (sin emitir)' }}</td></tr>
        <tr><th>Fecha de emisión</th><td>{{ $report->issued_at?->format('d/m/Y H:i') ?? '—' }}</td></tr>
        <tr><th>Elaborado con</th><td>{{ $companyName }}</td></tr>
    </table>
</div>
...
<table>
    <tr><th style="width:30%">Nombre (SysName)</th><td>{{ $summary['sysname'] ?? '—' }}</td></tr>
    <tr><th>Modelo</th><td>{{ $summary['system_type'] ?? '—' }}</td></tr>
    <tr><th>MAC del sistema</th><td>{{ $summary['system_mac'] ?? '—' }}</td></tr>
    <tr>
        ...
        <td>
            ...
            @else
                @foreach ($serials as $unit => $serial)
                    {{ $unit }}: <b>{{ $serial }}</b>@if (! $loop->last)<br>@endif
                @endforeach
            @endif
        </td>
    </tr>
    <tr><th>Versión EXOS</th><td>{{ $summary['exos_version'] ?? '—' }} @if(!empty($summary['firmware_build_date']))<span class="muted">(compilada: {{ $summary['firmware_build_date'] }})</span>@endif</td></tr>
    <tr><th>BootROM</th><td>{{ $summary['bootrom'] ?? '—' }}</td></tr>
    <tr><th>Licencia</th><td>{{ $summary['license'] ?? '—' }}</td></tr>
    <tr><th>Uptime</th><td>{{ $summary['uptime_text'] ?? '—' }}</td></tr>
    <tr><th>Boot count</th><td>{{ $summary['boot_count'] ?? '—' }}</td></tr>
    <tr><th>Stack</th><td>{{ !empty($summary['is_stack']) ? 'Sí' : 'No' }}</td></tr>
    <tr><th>Archivo analizado</th><td>{{ $capture->original_filename }} <span class="muted">(SHA-256: {{ substr($capture->file_hash, 0, 16) }}…)</span></td></tr>
</table>
    ...
    @foreach (array_chunk($areas, 5, true) as $chunk)
        <tr>
            @foreach ($chunk as $area)
                <td class="area-cell" style="width:20%; background: {{ App\Services\Reporting\AreaStatusService::pdfColor($area['status']) }};">
                    {{ $area['label'] }}<br>
...
<table style="border:none;">
    @foreach ($severityMeta as $key => $meta)
        ...
        <tr>
            <td style="border:none; width:90px;"><span class="badge" style="background: {{ $meta['color'] }};">{{ $meta['label'] }}</span></td>
            <td style="border:none;">
                <div class="bar-track">
                    <div class="bar" style="background: {{ $meta['color'] }}; width: {{ max(2, round($count / $maxCount * 100)) }}%;"></div>
                </div>
            </td>
...
@forelse ($findings as $i => $finding)
    <div class="finding">
        <h3>
            ...
            @if ($finding->is_manual)<span class="badge" style="background:#7c3aed;">Manual</span>@endif
        </h3>
        <p class="muted" style="margin:0 0 6px;">
            ...
            @if ($finding->status !== App\Enums\FindingStatus::Open) · Estado: {{ $finding->status->label() }} @endif
        </p>
        ...
        @foreach ($finding->attachments as $attachment)
            @if ($attachment->type === 'image' && is_file(storage_path('app/public/'.$attachment->path)))
                ...
                @if ($attachment->caption)<div class="foot-note">{{ $attachment->caption }}</div>@endif
            @elseif ($attachment->original_filename)
                <div class="foot-note">Adjunto: {{ $attachment->original_filename }} {{ $attachment->caption ? '— '.$attachment->caption : '' }}</div>
            @endif
        @endforeach
    </div>
@empty
